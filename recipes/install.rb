...
bash 'compile_and_install_s3fs' do
  ...
  code <<-EOH
    ...
    #{'export PKG_CONFIG_PATH=/usr/lib/pkgconfig:/usr/lib64/pkgconfig' if platform_family?('rhel')}
    ./configure --prefix=/usr/local
    make && make install
  EOH
  not_if do
    begin
      `s3fs --version`.to_s.split("\n").first.to_s.split.last == s3fs_version.to_s
    rescue Errno::ENOENT
      false
    end
  end
  if node['s3fs_fuse']['bluepill'] && File.exist?(File.join(node['bluepill']['conf_dir'], 's3fs.pill'))
